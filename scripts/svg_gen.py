...
def parse_circle(circle):
    return f'        p.addEllipse({f32(circle["@cx"])}f, {f32(circle["@cy"])}f, {f32(circle["@r"])}f, {f32(circle["@r"])}f);\n'
...
def my_parse_path(path) -> str:
    p = ''
    for thing in parse_path(path['@d']):
        match thing:
            case Move():
                continue
            ...
            case Arc() as var:
                ...
                c = np.cos(var.rotation)
                s = np.sin(var.rotation)
                A = np.array([[ c,  s],
                              [-s,  c]])
                ...
                p = f"{p}        p.addCentredArc({f32(cxp)}f, {f32(cyp)}f, {f32(radius[0])}f, {f32(radius[1])}f, {f32(var.rotation)}f, {f32(theta_1)}f, {f32(theta_2)}f);\n"
    ...
    for key in d['svg']:
        ...
        match key:
            ...
            case 'rect':
                w = f32(val['@width'])
                h = f32(val['@height'])
                r = f'    juce::Rectangle<float>(0.f, 0.f, {w}f, {h}f),\n'
                continue
            case 'line':
                if is_list:
                    for line in val:
...
def svg_to_cpp(file: str) -> Tuple[str, str]:
    ...
    hpp = f'{hpp}stoej::Icon {name}_ico;\n\n'
    cpp = f'{cpp}auto {name}_ico = stoej::Icon(\n'
